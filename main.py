...
from Point_clouds.point_cloud1 import point_cloud
from Point_clouds.point_cloud2 import point_cloudb
from Point_clouds.ply_cloud import ply_cloud
...
def rgb_img():
    ...
    #img4.save("DenseDepth/pic17.png")
...
def depthmap():
    ...
    dep = np.load("DenseDepth/depthmon7.npy")
    dep = dep[0, :, :, 0]
    dep2 = np.load("DenseDepth/depthmon17.npy")
    dep2 = dep2[0, :, :, 0]
...
def target4():
    ...
    rotation_matrix, trans1, Matrix = matrix(img_bin)
    pcd2 = point_cloudb(rotation_matrix, trans1, Matrix, depth_map2, img7, depth, pcd, fx, fy)
    #threading.Thread(target=lambda:matrix(img_bin)).start()
def target5():
    threading.Thread(target=lambda:ply_cloud(depth)).start()
    #threading.Thread(target=lambda:icp(pcd, pcd2)).start()
