# Written by DenseDepth & modified by Hayatu Abdullahi
...
class Window(QtWidgets.QWidget):
    ...
    def loadImageFile(self):
        self.capture = None
        filename = QtWidgets.QFileDialog.getOpenFileName(None, 'Select image', '', self.tr('Image files (*.jpg *.jpeg *.png)'))[0]
        print('Image Type:', type(filename))
        #img2 = Image.open(filename)
        #img3 = np.asarray(img2)
        #img3 =cv2.resize(img2,(640,480))
        #img4 = np.Imagefromarray(img3)
        #img4.save("pic.png")
        #img5 = Image.open("pic.png")
        img = QtGui.QImage(filename).scaledToHeight(rgb_height)
        print('Image Type 2:', type(img))
        #print(img.shape)
    ...
    def updateCloud(self):
        ...
        print('Shape:', self.glWidget.rgb.shape)
        print('Type:', type(self.glWidget.rgb.shape))
        ...
        if self.model: 
            with graph.as_default():
                ...
                np.save('depthmon_shoe.npy', depth)
                #depth.save("depthmon.npy")
            coloredDepth = (plasma(depth[0,:,:,0])[:,:,:3] * 255).astype('uint8')
            #print('Colored Depth Shape:', coloredDepth.shape)
            #print('Colored Depth Type:', type(coloredDepth))
            #np.save('depthmonc.npy', coloredDepth)
            #coloredDepth.save("depthmonc.npy")
            self.outputViewer.setPixmap(QtGui.QPixmap.fromImage(np_to_qimage(coloredDepth)))
        ...
        #print(coloredDepth)
        #print(coloredDepth.type)
        #depthmon = Image.fromarray(coloredDepth)
        #depthmon.save("depthmon.png")
        #coloredDepth.save("depthmon.bin")
        #np.save("depthmon", coloredDepth)
